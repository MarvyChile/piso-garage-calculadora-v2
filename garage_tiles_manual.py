...
# Modelos A–G - Canvas manual con cuadrícula visible (sin necesidad de parches).
# Requisitos:
#   streamlit>=1.18
...
# ─── Util para convertir PIL.Image a data-URL (evita image_to_url) ────  ### NEW
...
if st.checkbox("Activar diseño manual"):
    ...
    data_url = img_to_dataurl(img)                       # ←——  ### NEW

    canvas = st_canvas(
        fill_color=colores[color_sel],
        stroke_width=0,
        background_image=data_url,                       # ←——  ### NEW
        height=rows*cell,
        width=cols*cell,
        drawing_mode="point",
