...
color_secundario = st.selectbox("Color secundario (para diseño decorativo)", lista_colores, index=lista_colores.index("Rojo"))
...
# Medidas ajustadas
largo_total = rows * 0.4
ancho_total = cols * 0.4
if incluir_bordillos:
    if "Arriba" in pos_bord:
        largo_total += 0.06
    if "Abajo" in pos_bord:
...
ax.text(cols/2, rows + 0.6, f"{ancho_total:.2f} m", ha='center', va='bottom', fontsize=10)
ax.text(cols + 0.6, rows/2, f"{largo_total:.2f} m", ha='left', va='center', rotation=90, fontsize=10)
