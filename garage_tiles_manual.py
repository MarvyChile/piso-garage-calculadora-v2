...
from st_aggrid import AgGrid, GridOptionsBuilder, JsCode   # ← nuevo

# ───────────────── Configuración ──────────────────
st.set_page_config(layout="centered")
st.title("Piso Garage – Calculadora V2  +  Diseño manual AgGrid")
...
# ── 6. Diseño manual con AgGrid ────────────────────
st.markdown("## Diseño manual (fondo real)")
...
if activar_manual:
    ...
    gb.configure_default_cell(editable=True)

    js_color_dict = str(colores).replace("'", "\"")  # convertir a JSON JS
    style_js = JsCode(f"""
        function(params) {{
            const colmap = {js_color_dict};
            if (params.value && colmap[params.value]) {{
                return {{
                    'backgroundColor': colmap[params.value],
                    'color': (params.value === 'Negro') ? '#fff' : '#000'
                }}
            }}
        }}
    """)
    ...
    grid_res = AgGrid(
        ...
        theme="streamlit",
        height=min(600, 30*rows+35)  # tamaño dinámico
    )

    if st.button("Aplicar diseño manual"):
        st.session_state.df = pd.DataFrame(grid_res.data)
...
for c,n in palmetas.items(): st.markdown(f"- **{c}:** {n}")
