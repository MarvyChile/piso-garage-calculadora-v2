...
# Función de ayuda: columnas o filas centrales (soporta par/impar)
...
df = st.session_state.df  # dataframe resultante
...
# Bordillos – uso de bucles for (NO comprensiones de lista)
bord_col = "#000000"
if incluir_bordillos:
    if "Arriba" in pos_bord:
        for x in range(cols):
            ax.add_patch(plt.Rectangle((x, rows), 1, 0.15, facecolor=bord_col, edgecolor=borde))
    if "Abajo" in pos_bord:
        for x in range(cols):
            ax.add_patch(plt.Rectangle((x, -0.15), 1, 0.15, facecolor=bord_col, edgecolor=borde))
    if "Izquierda" in pos_bord:
        for y in range(rows):
            ax.add_patch(plt.Rectangle((-0.15, y), 0.15, 1, facecolor=bord_col, edgecolor=borde))
    if "Derecha" in pos_bord:
        for y in range(rows):
            ax.add_patch(plt.Rectangle((cols, y), 0.15, 1, facecolor=bord_col, edgecolor=borde))

# Esquineros
if incluir_esquineros:
    s = 0.15
    for cx, cy in [(0, 0), (0, rows), (cols, 0), (cols, rows)]:
        ax.add_patch(plt.Rectangle((cx - s/2, cy - s/2), s, s, facecolor=bord_col, edgecolor=borde))
